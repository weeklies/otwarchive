...
describe "rake After:add_new_work_dates" do
  context "for a work without new dates" do
    date1 = Time.utc(2024, 1, 29)
    date2 = Time.utc(2024, 1, 30)
    let!(:old_work) do
      travel_to(date1)
    ...
    it "does not change other datetime fields" do
      travel_back

      subject.invoke
      old_work.reload

      expect(old_work.published_at).to eq(date1.to_date)
      expect(old_work.created_at).to eq(date1.to_date)
      expect(old_work.revised_at).to eq(date2.to_date)
      expect(old_work.updated_at).to eq(date2.to_date)
    end

    it "fills posted_at and changed_at from existing date fields" do
      travel_back

      subject.invoke
      old_work.reload

      expect(old_work.posted_at).to eq(old_work.published_at)
      expect(old_work.changed_at).to eq(old_work.revised_at)
    end
  end
end

describe "rake After:add_new_chapter_date" do
  context "for a chapter without posted_at" do
    date1 = Time.utc(2024, 1, 29)
    date2 = Time.utc(2024, 1, 30)
    ...
    it "does not change other datetime fields" do
      ...
      expect(old_chapter.published_at).to eq(date1.to_date)
      expect(old_chapter.created_at).to eq(date1.to_date)
      expect(old_chapter.updated_at).to eq(date2.to_date)
    end

    it "fills posted_at from existing published_at" do
      travel_back

      subject.invoke
      old_chapter.reload

      expect(old_chapter.posted_at).to eq(old_chapter.published_at)
    end
  end
end
